	...
	public boolean isValid(Object value, String invalidValue,
		...
		// BigDecimal
		{
			...
			if (decimalLength > m_maxDecimalLength) {
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_RIGHT_OF_DECIMAL_TOO_LONG
			...
			if (nonDecimalLength > m_maxNonDecimalLength) {
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_LEFT_OF_DECIMAL_TOO_LONG
								.create(MessageContextFactory.MAX_LENGTH
			...
			if (!m_allowZero
				...
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_ZERO_NOT_ALLOWED.create()));
			}
			...
			if (!m_allowNegative
				...
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_NEGATIVE_NOT_ALLOWED
								.create()));
			}
		} else
		// Money was not able to parse invalidValue into a BigDecimal
		{
			...
			if (invalidValue.equals(".")) {
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_DOT_WITH_NO_NUMBERS
								.create()));
			}
			...
			if (invalidValue.indexOf(".") != invalidValue.lastIndexOf(".")) {
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_MULTIPLE_DOTS.create()));
			}
			...
			if (m_allowNegative
				...
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_MULTIPLE_DASHES.create()));
			}
			...
			if (m_allowNegative && (invalidValue.indexOf("-") != -1)
				...
				valid = false;
				receiver.addExceptionMessage(getMessageFactory().create(
						MessageContextFactory.MONEY_DASH_NOT_FIRST_CHARACTER
								.create()));
			}

...
public class DoubleButton2Strategy extends AbstractStrategyRule {
	...
	private final static Logger _log = LoggerFactory
			.getLogger(DoubleButton2Strategy.class);
	...
	public DoubleButton2Strategy(BrokerModel brokerManagerModel,
			...
			if (this.isThereOpenPosition()) {
				...
				if( checkingForC ){
					...
					if( currentCandleItem.getClose() >= (candlePositionB.getClose() - 0.1) && 
							currentCandleItem.getClose() <= (candlePositionB.getClose() + 0.1)){
						System.out.println("SE ENCONTRO EL PUNTO C EN " + currentCandleItem.getPeriod());
						Money limitPrice = new Money( currentCandleItem.getClose() );
						Money auxStopPrice = new Money((currentCandleItem.getClose() * 1.25));
				...
				if( startPeriod.isAfter(fechaApertura) && startPeriod.isBefore(fechaApertura2pm) ){
					...
					if( candlePositionA == null ){
						...
						// Revisamos si B sube 1%
						double costCloseUp1 = candlePositionB.getClose() * 1.01;
						if( currentCandleItem.getClose() >= costCloseUp1 ){
							// Si la posicion actual es mayor igual a B + 1%, compramos
							Money limitPrice = new Money( currentCandleItem.getClose() );
							Money auxStopPrice = new Money((currentCandleItem.getClose() * 1.25));
							this.createOrder(this.getTradestrategy().getContract(),
									Action.BUY, OrderType.STPLMT, limitPrice, auxStopPrice, cantidadCompra, false, true);
							...
							_log.info(" * ******************************************** *");
							return;
						}else if(currentCandleItem.getClose() < candlePositionB.getClose() ){
							checkingForC = true;
						}else{
							isStrategyInValid = true;
						}
